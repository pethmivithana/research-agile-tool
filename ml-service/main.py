...
try:
    ...
    print("⚠️ TabNet not installed. Quality model will be disabled.")
...
def load_models():
    ...
    # C. SCHEDULE (New Artifact-Based Loader)
    try:
        ...
        if os.path.exists(sch_model_path) and os.path.exists(sch_art_path):
            ...
            print("✅ Schedule Risk Model & Artifacts Loaded (9 Features)")
        else:
            print(f"❌ Schedule Model missing: {sch_model_path}")
    except Exception as e: print(f"❌ Schedule Load Error: {e}")

    # D. QUALITY
    if TABNET_AVAILABLE:
        try:
            q_path = os.path.join(MODELS_DIR, "tabnet_quality_model.zip")
        ...
        except Exception as e: print(f"❌ Quality Load Error: {e}")

# --- 5. PREDICTION LOGIC ---

def safe_transform(encoder, value, default=0):
    """Safely transforms a value using a LabelEncoder, handling unseen labels."""
    try:
...
def predict_schedule(data: TicketData):
    ...
    try:
        ...
        # 2. Use Saved Encoders (Crucial for Accuracy)
        t_c = safe_transform(assets["le_type"], data.issue_type)
        p_c = safe_transform(assets["le_prio"], data.priority)
        
        # 3. Construct Raw Vector (9 Features)
        # Order must match risk_artifacts.pkl['feature_names_in_']
        ...
        # 4. Use Imputer (Handles NaNs if any)
        if assets["imputer"]:
            final_vec = assets["imputer"].transform(raw_vec)
        else:
            final_vec = raw_vec
        ...
        # Mapping from your notebook: {0: 'Critical', 1: 'High', 2: 'Low', 3: 'Medium'}
        # Check specific mapping if possible, but this is standard alphabetical for LabelEncoder usually.
        # However, XGBoost classes are 0-indexed.
        labels = ["Critical Risk", "High Risk", "Low Risk", "Medium Risk"] 
        
        # Note: If your notebook had a specific mapping, adjust labels above.
        # Based on typical alphabetical sorting: Critical(0), High(1), Low(2), Medium(3)
...
def predict_quality(data: TicketData):
    ...
    try:
        ...
        vec = np.array([[
            float(data.story_points), float(data.total_links), float(data.total_comments),
            float(data.author_workload_14d), comp, float(p_c)
        ]])
        probs = assets["model"].predict_proba(vec)[0]
        return ("High" if probs[1] > 0.5 else "Low"), float(probs[1])
    except Exception as e:
...
def analyze_impact(data: TicketData):
    ...
    print(f"   Results -> Effort: {eff_h:.1f}h | Sched: {sch_lbl} | Prod: {prod_d:.1f}d | Qual: {qual_lbl}")
