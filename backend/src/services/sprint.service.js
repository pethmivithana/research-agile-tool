...
export async function autoDatesFromDuration(duration) {
  const start = new Date();
  const weeks = { '1w': 1, '2w': 2, '3w': 3, '4w': 4 }[duration] || 0;
  const end = new Date(start);
  end.setDate(end.getDate() + weeks * 7);
  return { startDate: weeks ? start : undefined, endDate: weeks ? end : undefined };
}
...
export async function completeSprint(sprintId) {
  ...
  await Promise.all(remaining.map(i => {
    i.sprint = next._id;
    if (i.type === 'Task' || i.type === 'Subtask') i.status = 'To Do';
    return i.save();
  }));
